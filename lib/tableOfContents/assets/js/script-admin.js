...
jQuery(document).ready(function($){
    ...
    // تغییر جدید: اگر toc_data.category تعریف نشده بود، ارور لاگ کن و پیش‌فرض ست کن
    if (typeof toc_data.category === 'undefined') {
        console.error('toc_data.category is undefined! Setting to default.');
        toc_data.category = 'default';
    }

    // Prepare initial tree data by requesting via AJAX
    function loadTree() {
        // تغییر جدید: قبل از ارسال، چک کن
        if (!toc_data.category) {
            alert('Category is missing! Cannot load tree.');
        ...
        return $.ajax({
            ...
            data: {
                ...
                category: toc_data.category // تغییر: category رو پاس می‌دیم
            }
        });
    }
    ...
    function extractTreeFromJsTree() {
        ...
        function convert(nodes) {
            var out = [];
            nodes.forEach(function(n){
                var post_id = n.data && n.data.post_id ? n.data.post_id : null; 
                var title = n.text;
                out.push({
                    post_id: post_id, 
                    title: title,
    ...
    $('#ctato-add-new-content').on('click', function(e){
        ...
        if (newTitle && newTitle.trim() !== '') { // تغییر: چک ساده برای خالی نبودن
            $.ajax({
                url: toc_data.ajax_url,
                method: 'POST',
                data: {
    ...
    $('#ctato-save-tree').on('click', function(e){
        ...
        // تغییر جدید: قبل از ارسال، چک کن
        if (!toc_data.category) {
            $('#ctato-save-status').text('Error: Category missing!');
            return;
        }
        console.log('Sending category to save: ' + toc_data.category); // تغییر جدید: لاگ category موقع سیو (در کنسول ببین)
